...
| INFRA-03 | No `.dockerignore` — full build context copied into both images | `portal/Dockerfile:6`, `notifier/Dockerfile:6` (`COPY . .`) | Bakes `.env` (DB password + live API key) and any local `sa-key.json` into image layers; if the image is ever pushed to a registry, the secrets ship with it permanently, recoverable from the layer even after a later layer "removes" it | high | TBD |
