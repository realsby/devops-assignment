...
| Secret scan caught a new file | [run 36035823093](https://github.com/realsby/devops-assignment/actions/runs/36035823093): gitleaks failed on the first push of `agent-traces/`, because the trace quoted the old burned keys. The values were masked in the trace and those commits rewritten, so the commit that run points at is no longer in history (see `agent-traces/README.md`) |
